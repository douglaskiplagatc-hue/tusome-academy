...
class User(UserMixin, db.Model):
    ...
    school_id = db.Column(
        db.Integer, db.ForeignKey("school_profiles.id"), nullable=True
    )
    school = db.relationship("SchoolProfile")
    # Relationships
...
class Teacher(db.Model):
    ...
    def __repr__(self):
        ...
        return f"<Teacher {user_name} (ID: {self.id})>"
...
class Grade(db.Model):
    ...
    )  # 'Exam 1', 'Exam 2', 'Exam 3', 'Summative'
    ...
    __table_args__ = (
        db.UniqueConstraint("student_id", "subject_id", "term", "year", "assessment_type", name="uq_grade_unique"),
    )

    @staticmethod
    def get_ncbe_level_and_points(percentage):
...
class FeeStatement(db.Model):
    ...
    @hybrid_property
    def balance(self):
        # Python-side calculation (for instances)
        return (self.amount_due or 0) - self.amount_paid

    @balance.expression
    def balance(cls):
        # SQL-side calculation using subquery
        total_paid = (
            select(func.coalesce(func.sum(FeePayment.amount_paid), 0))
            .where(FeePayment.fee_statement_id == cls.id)
            .scalar_subquery()
        )
        return cls.amount_due - total_paid
...
class StaffSalary(db.Model):
    ...
    staff_id = db.Column(
        db.Integer,
        db.ForeignKey("employees.id", name="fk_staff_salaries_employee"),
        nullable=False,
    )

    __table_args__ = (
        db.UniqueConstraint("staff_id", "month", "year", name="uq_staff_salary_period"),
    )
    ...
    staff = db.relationship(
        "Employee", foreign_keys=[staff_id], backref="salary_records"
    )
...
class SchoolProfile(db.Model):
    ...
    school_code = db.Column(db.String(50), unique=True)
    motto = db.Column(db.String(255))
    ...
    school_vision = db.Column(db.Text)  # Add this
    school_mission = db.Column(db.Text)
    website = db.Column(db.String(100))
    logo_url = db.Column(db.String(255))
    favicon_url = db.Column(db.String(255))
    primary_color = db.Column(db.String(20), default="#006B3F")
    secondary_color = db.Column(db.String(20), default="#0047AB")
    school_level = db.Column(db.String(50), default="junior")  # junior, senior, primary
    established_year = db.Column(db.Integer)
    principal_name = db.Column(db.String(100))
    principal_signature = db.Column(db.String(255))
    social_facebook = db.Column(db.String(500))
    social_twitter = db.Column(db.String(500))
    social_instagram = db.Column(db.String(500))
    social_linkedin = db.Column(db.String(500))
    social_youtube = db.Column(db.String(500))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    )
...
class SchoolLevel(db.Model):
    """School level configuration (Junior/Senior/Primary)"""
    ...
    grades = db.Column(db.JSON, default=[])  # e.g., [7,8,9] for junior
    description = db.Column(db.Text)
    is_active = db.Column(db.Boolean, default=True)

    def __init__(self, name, code, grades=None, description=None):
        self.name = name
        self.code = code
...
class LevelSubject(db.Model):
    """Subjects offered per school level"""
...
# Initialize default school levels
def init_school_levels():
    levels = [
        SchoolLevel(
            name="Junior School",
            code="junior",
            grades=[7, 8, 9],
            description="CBC Junior School (Grades 7-9)",
        ),
        SchoolLevel(
            name="Senior School",
            code="senior",
            grades=[10, 11, 12],
            description="CBC Senior School (Grades 10-12)",
        ),
        SchoolLevel(
            name="Primary School",
            code="primary",
    ...
    for level in levels:
        existing = SchoolLevel.query.filter_by(code=level.code).first()
        if not existing:
...
# Initialize default subjects per level
def init_level_subjects():
    subjects_config = {
        "junior": [
            {"name": "English", "code": "ENG", "core": True},
            {"name": "Kiswahili", "code": "KIS", "core": True},
            {"name": "Mathematics", "code": "MAT", "core": True},
            {"name": "Science", "code": "SCI", "core": True},
            {"name": "Social Studies", "code": "SST", "core": True},
            {"name": "Religious Education", "code": "RE", "core": True},
            {"name": "Agriculture", "code": "AGR", "core": False},
            {"name": "Computer Science", "code": "COM", "core": False},
            {"name": "Business Studies", "code": "BUS", "core": False},
            {"name": "French", "code": "FRN", "core": False},
            {"name": "German", "code": "GER", "core": False},
        ],
        "senior": [
            {"name": "English", "code": "ENG", "core": True},
            {"name": "Kiswahili", "code": "KIS", "core": True},
            ...
            {"name": "Chemistry", "code": "CHE", "core": False},
            {"name": "Physics", "code": "PHY", "core": False},
            {"name": "History", "code": "HIS", "core": False},
            {"name": "Geography", "code": "GEO", "core": False},
            {"name": "CRE", "code": "CRE", "core": False},
            {"name": "Computer Studies", "code": "CST", "core": False},
            {"name": "Business Studies", "code": "BST", "core": False},
        ],
        "primary": [
            {"name": "English", "code": "ENG", "core": True},
            {"name": "Kiswahili", "code": "KIS", "core": True},
            ...
            {"name": "Science", "code": "SCI", "core": True},
            {"name": "Social Studies", "code": "SST", "core": True},
            {"name": "CRE", "code": "CRE", "core": True},
            {"name": "Art & Craft", "code": "ART", "core": False},
            {"name": "Music", "code": "MUS", "core": False},
            {"name": "Physical Education", "code": "PE", "core": False},
        ],
    }

    for level_code, subjects in subjects_config.items():
        for idx, subject in enumerate(subjects):
            existing = LevelSubject.query.filter_by(
                level_code=level_code, subject_name=subject["name"]
            ).first()
            if not existing:
...
class Assignment(db.Model):
    ...
    school = db.relationship("SchoolProfile")
class BulkImportResult(db.Model):
    __tablename__ = "bulk_import_results"
